...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Pop address (e.g. %1)").arg("ugb46GtdASZqV4vZnLt4acRtJHXWrhpwXi"));
#endif
...
{
    // return if URI is not valid or is no pop: URI
    if(!uri.isValid() || uri.scheme() != QString("pop"))
        return false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    // Convert pop:// to pop:
    //
    //    Cannot handle this later, because pop:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("pop://", Qt::CaseInsensitive))
    {
        uri.replace(0, 7, "pop:");
    }
...
void openConfigfile()
{
    ...
    /* Open pop.conf with the associated application */
...
{
    boost::filesystem::path pathConfig = GetPopnodeConfigFile();

    /* Open popnode.conf with the associated application */
    if (boost::filesystem::exists(pathConfig))
        QDesktopServices::openUrl(QUrl::fromLocalFile(boostPathToQString(pathConfig)));
}
...
boost::filesystem::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Pop.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Pop (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Pop (%s).lnk", chain);
}
...
boost::filesystem::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "pop.desktop";
    return GetAutostartDir() / strprintf("pop-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a pop.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Pop Core app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Pop Core app to startup item list

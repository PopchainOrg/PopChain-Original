    ...
    platformStyle(platformStyle)
{
    ...
    // QString windowTitle = tr("Pop Core") + " - ";
    QString windowTitle = tr("Pop Core");
#ifdef ENABLE_WALLET
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Pop address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and pop: URIs)"));
...
#ifdef ENABLE_WALLET
    QSettings settings;
    if (settings.value("fShowPopnodesTab").toBool()) {
        popnodeAction = new QAction(QIcon(":/icons/" + theme + "/popnodes"), tr("&Popnodes"), this);
        popnodeAction->setStatusTip(tr("Browse popnodes"));
        popnodeAction->setToolTip(popnodeAction->statusTip());
        popnodeAction->setCheckable(true);
#ifdef Q_OS_MAC
        popnodeAction->setShortcut(QKeySequence(Qt::CTRL + Qt::Key_5));
#else
        popnodeAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_5));
#endif
        tabGroup->addAction(popnodeAction);
        connect(popnodeAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));
        connect(popnodeAction, SIGNAL(triggered()), this, SLOT(gotoPopnodePage()));
    }
    ...
    aboutAction = new QAction(/*QIcon(":/icons/" + theme + "/about"),*/ tr("&About Pop Core"), this);
    aboutAction->setStatusTip(tr("Show information about Pop Core"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for Pop Core"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Pop addresses to prove you own them"));
    verifyMessageAction = new QAction(/*QIcon(":/icons/" + theme + "/transaction_0"), */tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Pop addresses"));
    ...
    openMNConfEditorAction = new QAction(/*QIcon(":/icons/" + theme + "/edit"), */tr("Open &Popnode Configuration File"), this);
    openMNConfEditorAction->setStatusTip(tr("Open Popnode configuration file"));    
    ...
    openAction->setStatusTip(tr("Open a Pop: URI or payment request"));

    showHelpMessageAction = new QAction(/*QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), */ tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    // showHelpMessageAction->setStatusTip(tr("Show the Pop Core help message to get a list with possible Pop Core command-line options"));
...
void BitcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        if (settings.value("fShowPopnodesTab").toBool())
        {
            toolbar->addAction(popnodeAction);
        }
...
void BitcoinGUI::setWalletActionsEnabled(bool enabled)
{
    ...
    QSettings settings;
    if (settings.value("fShowPopnodesTab").toBool() && popnodeAction) {
        popnodeAction->setEnabled(enabled);
    }
...
void BitcoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
    ...
    QString toolTip = tr("Pop Core client") + " " + networkStyle->getTitleAddText();
...
void BitcoinGUI::gotoPopnodePage()
{
    QSettings settings;
    if (settings.value("fShowPopnodesTab").toBool()) {
        popnodeAction->setChecked(true);
        if (walletFrame) walletFrame->gotoPopnodePage();
    }
}
...
void BitcoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress)
{
    ...
    if(!popnodeSync.IsBlockchainSynced())
    {
...
void BitcoinGUI::setAdditionalDataSyncProgress(double nSyncProgress)
{
    ...
    if(popnodeSync.IsBlockchainSynced())
    {
        ...
        strSyncStatus = QString(popnodeSync.GetSyncStatus().c_str());
...
{
    QString strTitle = tr("Pop Core"); // default title

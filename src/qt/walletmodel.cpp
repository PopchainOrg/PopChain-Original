...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
...

    {
        ...
        if(recipients[0].fUseInstantSend && total > forkManager.GetForkValue(FORK_5_INSTANTSEND_MAX_VALUE)*COIN){
            Q_EMIT message(tr("Send Coins"), tr("InstantSend doesn't support sending values that high yet. Transactions are currently limited to %1 PCH.").arg(forkManager.GetForkValue(FORK_5_INSTANTSEND_MAX_VALUE)),
        ...
        if(recipients[0].fUseInstantSend) {
            if(newTx->GetValueOut() > forkManager.GetForkValue(FORK_5_INSTANTSEND_MAX_VALUE)*COIN) {
                Q_EMIT message(tr("Send Coins"), tr("InstantSend doesn't support sending values that high yet. Transactions are currently limited to %1 PCH.").arg(forkManager.GetForkValue(FORK_5_INSTANTSEND_MAX_VALUE)),
                             CClientUIInterface::MSG_ERROR);
                return TransactionCreationFailed;
            }

    ...
    filter(0)
{
    ...
    // Disable any PS UI for popnode or when autobackup is disabled or failed for whatever reason
    if(fPopNode || nWalletBackups <= 0){
...
{
    if(!fLiteMode && !fPopNode) disconnect(timer, SIGNAL(timeout()), this, SLOT(privateSendStatus()));
    delete ui;
}
...
{
    if(!popnodeSync.IsBlockchainSynced() || ShutdownRequested()) return;

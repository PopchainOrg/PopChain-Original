...
void OptionsModel::Init(bool resetSettings)
{
    ...
    if (!settings.contains("fShowPopnodesTab"))
        settings.setValue("fShowPopnodesTab", popnodeConfig.getCount());
    ...
    if (!settings.contains("nPrivateSendAmount")) {
        ...
        if (!settings.contains("nAnonymizePopAmount"))
            settings.setValue("nPrivateSendAmount", DEFAULT_PRIVATESEND_AMOUNT);
        else
            settings.setValue("nPrivateSendAmount", settings.value("nAnonymizePopAmount").toInt());
    }
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in pop.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case ShowPopnodesTab:
            return settings.value("fShowPopnodesTab");
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case SpendZeroConfChange:
            ...
            break;
        case ShowPopnodesTab:
            if (settings.value("fShowPopnodesTab") != value) {
                settings.setValue("fShowPopnodesTab", value);
                setRestartRequired(true);
            }
            break;
        case ShowAdvancedPSUI:

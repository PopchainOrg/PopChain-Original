...
void Intro::pickDataDirectory()
{
    ...
    if(!fs::exists(GUIUtil::qstringToBoostPath(dataDir)) || GetBoolArg("-choosedatadir", DEFAULT_CHOOSE_DATADIR) || dataDirDefaultCurrent != dataDirDefaultSettings)
    {
        ...
        while(true)
        {
            ...
            try {
                CreateDirectory(GUIUtil::qstringToBoostPath(dataDir));
                break;
            } catch (const fs::filesystem_error&) {
                QMessageBox::critical(0, tr("Pop Core"),
     ...
     * override -datadir in the pop.conf file in the default data directory
     * (to be consistent with popd behavior)
     */

...
// A helper object for signing messages from Popnodes
...
public:
     ...
     *     1) we have an active Popnode,
     *     2) we have a valid Popnode private key,
     *     3) we signed the message successfully, and
     *     4) we verified the message successfully
     */
    bool Sign();
    /// Check if we have a valid Popnode address
    bool CheckSignature(const CPubKey& pubKeyPopnode);
    ...
    std::string ToString()
    {
        return strprintf("nDenom=%d, nTime=%lld, fReady=%s, fTried=%s, popnode=%s",
...
{
public:
    /// Is the input associated with this public key? (and there is 10000 PCH - checking if valid popnode)
...
private:
    ...
    // Keep track of the used Popnodes
    std::vector<CTxIn> vecPopnodesUsed;
    ...
    std::vector<CDarkSendEntry> vecEntries; // Popnode/clients entries
    ...
    /// Get the denominations for a specific amount of pop.
    ...
    /// As a client, submit part of a future mixing transaction to a Popnode to start the process
    ...
    /// Get Popnode updates about the progress of mixing

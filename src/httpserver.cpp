...
//! List of supops to allow RPC connections from
static std::vector<CSubNet> rpc_allow_supops;
...
static bool ClientAllowed(const CNetAddr& netaddr)
{
    if (!netaddr.IsValid())
        return false;
    BOOST_FOREACH (const CSubNet& supop, rpc_allow_supops)
        if (supop.Match(netaddr))
            return true;
    return false;
}
...
{
    rpc_allow_supops.clear();
    rpc_allow_supops.push_back(CSubNet("127.0.0.0/8")); // always allow IPv4 local supop
    rpc_allow_supops.push_back(CSubNet("::1"));         // always allow IPv6 localhost
    if (mapMultiArgs.count("-rpcallowip")) {
        const std::vector<std::string>& vAllow = mapMultiArgs["-rpcallowip"];
        BOOST_FOREACH (std::string strAllow, vAllow) {
            CSubNet supop(strAllow);
            if (!supop.IsValid()) {
                uiInterface.ThreadSafeMessageBox(
                    strprintf("Invalid -rpcallowip supop specification: %s. Valid are a single IP (e.g. 1.2.3.4), a network/netmask (e.g. 1.2.3.4/255.255.255.0) or a network/CIDR (e.g. 1.2.3.4/24).", strAllow),
                    "", CClientUIInterface::MSG_ERROR);
                return false;
            }
            rpc_allow_supops.push_back(supop);
        }
    }
    std::string strAllowed;
    BOOST_FOREACH (const CSubNet& supop, rpc_allow_supops)
        strAllowed += supop.ToString() + " ";

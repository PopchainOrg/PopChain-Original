...
CSubNet::CSubNet(const std::string &strSupop, bool fAllowLookup)
{
    size_t slash = strSupop.find_last_of('/');
    std::vector<CNetAddr> vIP;
    ...
    std::string strAddress = strSupop.substr(0, slash);
    if (LookupHost(strAddress.c_str(), vIP, 1, fAllowLookup))
    {
        network = vIP[0];
        if (slash != strSupop.npos)
        {
            std::string strNetmask = strSupop.substr(slash + 1);
            int32_t n;

...
/*
    At 15 signatures, 1/2 of the popnode network can be owned by
...
private:
    ...
    //track popnodes who voted with no txreq (for DOS protection)
    std::map<COutPoint, int64_t> mapPopnodeOrphanVotes; // mn outpoint - time
    ...
    int64_t GetAveragePopnodeOrphanVoteTime();
...
private:
    uint256 txHash;
    COutPoint outpoint;
    COutPoint outpointPopnode;
    std::vector<unsigned char> vchPopnodeSignature;
    // local memory only
...
public:
    ...
    CTxLockVote(const uint256& txHashIn, const COutPoint& outpointIn, const COutPoint& outpointPopnodeIn) :
        txHash(txHashIn),
        outpoint(outpointIn),
        outpointPopnode(outpointPopnodeIn),
        vchPopnodeSignature(),
        nConfirmedHeight(-1),
        nTimeCreated(GetTime())
        {}
    ...
    inline void SerializationOp(Stream& s, Operation ser_action, int nType, int nVersion) {
        READWRITE(txHash);
        READWRITE(outpoint);
        READWRITE(outpointPopnode);
        READWRITE(vchPopnodeSignature);
    }
    ...
    COutPoint GetPopnodeOutpoint() const { return outpointPopnode; }
...
private:
    ...
    std::map<COutPoint, CTxLockVote> mapPopnodeVotes; // popnode outpoint - vote
...
public:
    ...
    bool HasPopnodeVoted(const COutPoint& outpointPopnodeIn) const;
    int CountVotes() const { return mapPopnodeVotes.size(); }
...
public:
    ...
    bool HasPopnodeVoted(const COutPoint& outpointIn, const COutPoint& outpointPopnodeIn);

...
{
    if(fLiteMode) return; // ignore all Pop related functionality
    ...
    if(strCommand == NetMsgType::DSACCEPT) {
        ...
        if(!fPopNode) {
            LogPrintf("DSACCEPT -- not a Popnode!\n");
        ...
        CPopnode* pmn = mnodeman.Find(activePopnode.vin);
        if(pmn == NULL) {
        ...
        CPopnode* pmn = mnodeman.Find(dsq.vin);
        if(pmn == NULL) return;

        if(!dsq.CheckSignature(pmn->pubKeyPopnode)) {
        ...
        if(dsq.fReady) {
            if(!pSubmittedToPopnode) return;
            if((CNetAddr)pSubmittedToPopnode->addr != (CNetAddr)pmn->addr) {
                LogPrintf("DSQUEUE -- message doesn't match current Popnode: pSubmittedToPopnode=%s, addr=%s\n", pSubmittedToPopnode->addr.ToString(), pmn->addr.ToString());
                return;
            }

            if(nState == POOL_STATE_QUEUE) {
                LogPrint("privatesend", "DSQUEUE -- PrivateSend queue (%s) is ready on popnode %s\n", dsq.ToString(), pmn->addr.ToString());
                SubmitDenominate();
            }
        } else {
            BOOST_FOREACH(CDarksendQueue q, vecDarksendQueue) {
                if(q.vin == dsq.vin) {
                    ...
                    LogPrint("privatesend", "DSQUEUE -- Popnode %s is sending WAY too many dsq messages\n", pmn->addr.ToString());
                    return;
                }
            }
            ...
            if(pmn->nLastDsq != 0 && nThreshold > mnodeman.nDsqCount) {
                LogPrint("privatesend", "DSQUEUE -- Popnode %s is sending too many dsq messages\n", pmn->addr.ToString());
                return;
            }
            mnodeman.nDsqCount++;
            ...
            LogPrint("privatesend", "DSQUEUE -- new PrivateSend queue (%s) from popnode %s\n", dsq.ToString(), pmn->addr.ToString());
            if(pSubmittedToPopnode && pSubmittedToPopnode->vin.prevout == dsq.vin.prevout) {
                dsq.fTried = true;
            }
        ...
        if(fPopNode) {
            // LogPrintf("DSSTATUSUPDATE -- Can't run on a Popnode!\n");
            return;
        }

        if(!pSubmittedToPopnode) return;
        if((CNetAddr)pSubmittedToPopnode->addr != (CNetAddr)pfrom->addr) {
            //LogPrintf("DSSTATUSUPDATE -- message doesn't match current Popnode: pSubmittedToPopnode %s addr %s\n", pSubmittedToPopnode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
        ...
        if(!fPopNode) {
            LogPrintf("DSSIGNFINALTX -- not a Popnode!\n");
            return;
        }
        ...
        if(fPopNode) {
            // LogPrintf("DSFINALTX -- Can't run on a Popnode!\n");
            return;
        }

        if(!pSubmittedToPopnode) return;
        if((CNetAddr)pSubmittedToPopnode->addr != (CNetAddr)pfrom->addr) {
            //LogPrintf("DSFINALTX -- message doesn't match current Popnode: pSubmittedToPopnode %s addr %s\n", pSubmittedToPopnode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
        ...
        if(fPopNode) {
            // LogPrintf("DSCOMPLETE -- Can't run on a Popnode!\n");
            return;
        }

        if(!pSubmittedToPopnode) return;
        if((CNetAddr)pSubmittedToPopnode->addr != (CNetAddr)pfrom->addr) {
            LogPrint("privatesend", "DSCOMPLETE -- message doesn't match current Popnode: pSubmittedToPopnode=%s  addr=%s\n", pSubmittedToPopnode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
...
std::string CDarksendPool::GetStatus()
{
    ...
    if((pCurrentBlockIndex && pCurrentBlockIndex->nHeight - nCachedLastSuccessBlock < nMinBlockSpacing) || !popnodeSync.IsBlockchainSynced())
    ...
    switch(nState) {
        ...
        case POOL_STATE_QUEUE:
            ...
            return strprintf(_("Submitted to popnode, waiting in queue %s"), strSuffix);;
        case POOL_STATE_ACCEPTING_ENTRIES:
            if(nEntriesCount == 0) {
                ...
                if(     nStatusMessageProgress % 70 <= 40) return strprintf(_("Submitted following entries to popnode: %u / %d"), nEntriesCount, GetMaxPoolTransactions());
                else if(nStatusMessageProgress % 70 <= 50) strSuffix = ".";
                else if(nStatusMessageProgress % 70 <= 60) strSuffix = "..";
                else if(nStatusMessageProgress % 70 <= 70) strSuffix = "...";
                return strprintf(_("Submitted to popnode, waiting for more entries ( %u / %d ) %s"), nEntriesCount, GetMaxPoolTransactions(), strSuffix);
            }
...
//
// Check the mixing progress and send client updates if a Popnode
//
...
{
    if(!fPopNode) return; // check and relay final tx only on popnode
    ...
    // create and sign popnode dstx transaction
    if(!mapDarksendBroadcastTxes.count(hashTx)) {
        CDarksendBroadcastTx dstx(finalTransaction, activePopnode.vin, GetAdjustedTime());
        dstx.Sign();
...
//
// How does this work? Messages to Popnodes come in via NetMsgType::DSVIN, these require a valid collateral
// transaction for the client to be able to enter the pool. This transaction is kept by the Popnode
...
/*
    ...
    allow endless transaction that would bloat Pop and make it unusable. To
...
void CDarksendPool::CheckTimeout()
{
    ...
    if(!fEnablePrivateSend && !fPopNode) return;
    ...
    int nLagTime = fPopNode ? 0 : 10000; // if we're the client, give the server a few extra seconds before resetting.
...
void CDarksendPool::CheckForCompleteQueue()
{
    ...
    if(nState == POOL_STATE_QUEUE && IsSessionReady()) {
        ...
        CDarksendQueue dsq(nSessionDenom, activePopnode.vin, GetTime(), true);
...
//
// Execute a mixing denomination via a Popnode.
// This is only ran from clients
//
bool CDarksendPool::SendDenominate(const std::vector<CTxIn>& vecTxIn, const std::vector<CTxOut>& vecTxOut)
{
    if(fPopNode) {
        LogPrintf("CDarksendPool::SendDenominate -- PrivateSend from a Popnode is not supported currently.\n");
        return false;
    }
    ...
    // we should already be connected to a Popnode
    if(!nSessionID) {
        LogPrintf("CDarksendPool::SendDenominate -- No Popnode has been selected yet.\n");
        UnlockCoins();
        SetNull();
        return false;
    }
...
// Incoming message from Popnode updating the progress of mixing
bool CDarksendPool::CheckPoolStateUpdate(PoolState nStateNew, int nEntriesCountNew, PoolStatusUpdate nStatusUpdate, PoolMessage nMessageID, int nSessionIDNew)
{
    ...
    strAutoDenomResult = _("Popnode:") + " " + GetMessageByID(nMessageID);

    // if rejected at any state
    if(nStatusUpdate == STATUS_REJECTED) {
        LogPrintf("CDarksendPool::CheckPoolStateUpdate -- entry is rejected by Popnode\n");
        UnlockCoins();
        SetNull();
        SetState(POOL_STATE_ERROR);
...
bool CDarksendPool::SignFinalTransaction(const CTransaction& finalTransactionNew, CNode* pnode)
{
    ...
    // push all of our signatures to the Popnode
    LogPrintf("CDarksendPool::SignFinalTransaction -- pushing sigs to the popnode, finalMutableTransaction=%s", finalMutableTransaction.ToString());
...
{
    if(!fEnablePrivateSend || fPopNode || !pCurrentBlockIndex) return false;
    ...
    if(mnodeman.size() == 0) {
        LogPrint("privatesend", "CDarksendPool::DoAutomaticDenominating -- No Popnodes detected\n");
        strAutoDenomResult = _("No Popnodes detected.");
        return false;
    }
    ...
    // If we've used 90% of the Popnode list then drop the oldest first ~30%
    int nThreshold_high = nMnCountEnabled * 0.9;
    int nThreshold_low = nThreshold_high * 0.7;
    LogPrint("privatesend", "Checking vecPopnodesUsed: size: %d, threshold: %d\n", (int)vecPopnodesUsed.size(), nThreshold_high);

    if((int)vecPopnodesUsed.size() > nThreshold_high) {
        vecPopnodesUsed.erase(vecPopnodesUsed.begin(), vecPopnodesUsed.begin() + vecPopnodesUsed.size() - nThreshold_low);
        LogPrint("privatesend", "  vecPopnodesUsed: new size: %d, threshold: %d\n", (int)vecPopnodesUsed.size(), nThreshold_high);
    }
    ...
    if(nLiquidityProvider || fUseQueue) {
        ...
        BOOST_FOREACH(CDarksendQueue& dsq, vecDarksendQueue) {
            ...
            CPopnode* pmn = mnodeman.Find(dsq.vin);
            if(pmn == NULL) {
                LogPrintf("CDarksendPool::DoAutomaticDenominating -- dsq popnode is not in popnode list, popnode=%s\n", dsq.vin.prevout.ToStringShort());
                continue;
            }
            ...
            vecPopnodesUsed.push_back(dsq.vin);

            LogPrintf("CDarksendPool::DoAutomaticDenominating -- attempt to connect to popnode from queue, addr=%s\n", pmn->addr.ToString());
            // connect to Popnode and submit the queue request
            CNode* pnode = ConnectNode((CAddress)pmn->addr, NULL, true);
            if(pnode) {
                pSubmittedToPopnode = pmn;
                ...
                strAutoDenomResult = _("Error connecting to Popnode.");
                continue;
            }
        }
    }
    ...
    while(nTries < 10) {
        CPopnode* pmn = mnodeman.FindRandomNotInVec(vecPopnodesUsed, MIN_PRIVATESEND_PEER_PROTO_VERSION);
        if(pmn == NULL) {
            LogPrintf("CDarksendPool::DoAutomaticDenominating -- Can't find random popnode!\n");
            strAutoDenomResult = _("Can't find random Popnode.");
            return false;
        }
        vecPopnodesUsed.push_back(pmn->vin);

        if(pmn->nLastDsq != 0 && pmn->nLastDsq + nMnCountEnabled/5 > mnodeman.nDsqCount) {
            LogPrintf("CDarksendPool::DoAutomaticDenominating -- Too early to mix on this popnode!"
                        " popnode=%s  addr=%s  nLastDsq=%d  CountEnabled/5=%d  nDsqCount=%d\n",
        ...
        LogPrintf("CDarksendPool::DoAutomaticDenominating -- attempt %d connection to Popnode %s\n", nTries, pmn->addr.ToString());
        CNode* pnode = ConnectNode((CAddress)pmn->addr, NULL, true);
        if(pnode) {
    ...
    strAutoDenomResult = _("No compatible Popnode found.");
    return false;
}
...
{
    if(!fPopNode || nSessionID != 0) return false;
    ...
    if(!fUnitTest) {
        ...
        CDarksendQueue dsq(nDenom, activePopnode.vin, GetTime(), false);
...
{
    if(!fPopNode || nSessionID == 0 || IsSessionReady()) return false;
...
std::string CDarksendPool::GetMessageByID(PoolMessage nMessageID)
{
    switch (nMessageID) {
        ...
        case ERR_MN_LIST:               return _("Not in the Popnode list.");
        case ERR_MODE:                  return _("Incompatible mode.");
        case ERR_NON_STANDARD_PUBKEY:   return _("Non-standard public key detected.");
        case ERR_NOT_A_MN:              return _("This is not a Popnode.");
        case ERR_QUEUE_FULL:            return _("Popnode queue is full.");
...
bool CDarksendQueue::Sign()
{
    ...
    if(!darkSendSigner.SignMessage(strMessage, vchSig, activePopnode.keyPopnode)) {
        LogPrintf("CDarksendQueue::Sign -- SignMessage() failed, %s\n", ToString());
        return false;
    }

    return CheckSignature(activePopnode.pubKeyPopnode);
}

bool CDarksendQueue::CheckSignature(const CPubKey& pubKeyPopnode)
{
    std::string strMessage = vin.ToString() + boost::lexical_cast<std::string>(nDenom) + boost::lexical_cast<std::string>(nTime) + boost::lexical_cast<std::string>(fReady);
    std::string strError = "";

    if(!darkSendSigner.VerifyMessage(pubKeyPopnode, vchSig, strMessage, strError)) {
        LogPrintf("CDarksendQueue::CheckSignature -- Got bad Popnode queue signature: %s; error: %s\n", ToString(), strError);
        return false;
    }
...
bool CDarksendBroadcastTx::CheckSignature(const CPubKey& pubKeyPopnode)
{
    std::string strMessage = tx.GetHash().ToString() + boost::lexical_cast<std::string>(sigTime);
    std::string strError = "";
...
void CDarksendPool::RelayIn(const CDarkSendEntry& entry)
{
    ...
    CNode* pnode = FindNode(pSubmittedToPopnode->addr);
    if(pnode != NULL) {
...
{
    if(fPopNode && (nStateNew == POOL_STATE_ERROR || nStateNew == POOL_STATE_SUCCESS)) {
        LogPrint("privatesend", "CDarksendPool::SetState -- Can't set state to ERROR or SUCCESS as a Popnode. \n");
        return;
    }
...
void CDarksendPool::UpdatedBlockTip(const CBlockIndex *pindex)
{
    ...
    if(!fLiteMode && popnodeSync.IsPopnodeListSynced()) {
        NewBlock();
    }
}
...
void ThreadCheckDarkSendPool()
{
    ...
    while (true)
    {
        ...
        popnodeSync.ProcessTick();

        if(popnodeSync.IsBlockchainSynced() && !ShutdownRequested()) {

            nTick++;

            // make sure to check all popnodes first
            mnodeman.Check();

            // check if we should activate or ping every few minutes,
            // slightly postpone first run to give net thread a chance to connect to some peers
            if(nTick % POPNODE_MIN_MNP_SECONDS == 15)
                activePopnode.ManageState();

            if(nTick % 60 == 0) {
                mnodeman.ProcessPopnodeConnections();

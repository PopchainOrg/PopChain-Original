...
// step 2) Top COutPointLock::SIGNATURES_TOTAL popnodes per each spent outpoint push "txvote" message
...
bool CInstantSend::ProcessTxLockRequest(const CTxLockRequest& txLockRequest)
{
    ...
    // Popnodes will sometimes propagate votes before the transaction is known to the client.
...
void CInstantSend::Vote(CTxLockCandidate& txLockCandidate)
{
    ...
    while(itOutpointLock != txLockCandidate.mapOutPointLocks.end()) {
        ...
        int n = mnodeman.GetPopnodeRank(activePopnode.vin, nLockInputHeight, MIN_INSTANTSEND_PROTO_VERSION);

        if(n == -1) {
            LogPrint("instantsend", "CInstantSend::Vote -- Unknown Popnode %s\n", activePopnode.vin.prevout.ToStringShort());
            ++itOutpointLock;
            continue;
        }
        ...
        if(n > nSignaturesTotal) {
            LogPrint("instantsend", "CInstantSend::Vote -- Popnode not in the top %d (%d)\n", nSignaturesTotal, n);
            ++itOutpointLock;
            continue;
        }
        ...
        if(itVoted != mapVotedOutpoints.end()) {
            BOOST_FOREACH(const uint256& hash, itVoted->second) {
                ...
                if(it2->second.HasPopnodeVoted(itOutpointLock->first, activePopnode.vin.prevout)) {
        ...
        CTxLockVote vote(txHash, itOutpointLock->first, activePopnode.vin.prevout);
...
bool CInstantSend::ProcessTxLockVote(CNode* pfrom, CTxLockVote& vote)
{
    ...
    if(it == mapTxLockCandidates.end()) {
        if(!mapTxLockVotesOrphan.count(vote.GetHash())) {
            ...
            LogPrint("instantsend", "CInstantSend::ProcessTxLockVote -- Orphan vote: txid=%s  popnode=%s new\n",
                    txHash.ToString(), vote.GetPopnodeOutpoint().ToStringShort());
            ...
            LogPrint("instantsend", "CInstantSend::ProcessTxLockVote -- Orphan vote: txid=%s  popnode=%s seen\n",
                    txHash.ToString(), vote.GetPopnodeOutpoint().ToStringShort());
        }
        ...
        int nPopnodeOrphanExpireTime = GetTime() + 60*10; // keep time data for 10 minutes
        if(!mapPopnodeOrphanVotes.count(vote.GetPopnodeOutpoint())) {
            mapPopnodeOrphanVotes[vote.GetPopnodeOutpoint()] = nPopnodeOrphanExpireTime;
        } else {
            int64_t nPrevOrphanVote = mapPopnodeOrphanVotes[vote.GetPopnodeOutpoint()];
            if(nPrevOrphanVote > GetTime() && nPrevOrphanVote > GetAveragePopnodeOrphanVoteTime()) {
                LogPrint("instantsend", "CInstantSend::ProcessTxLockVote -- popnode is spamming orphan Transaction Lock Votes: txid=%s  popnode=%s\n",
                        txHash.ToString(), vote.GetPopnodeOutpoint().ToStringShort());
    ...
    if(it1 != mapVotedOutpoints.end()) {
        BOOST_FOREACH(const uint256& hash, it1->second) {
            if(hash != txHash) {
                ...
                std::map<uint256, CTxLockCandidate>::iterator it2 = mapTxLockCandidates.find(hash);
                if(it2->second.HasPopnodeVoted(vote.GetOutpoint(), vote.GetPopnodeOutpoint())) {
                    // yes, it did, refuse to accept a vote to include the same outpoint in another tx
                    // from the same popnode.
                    // TODO: apply pose ban score to this popnode?
                    ...
                    // pose ban score too.
                    LogPrintf("CInstantSend::ProcessTxLockVote -- popnode sent conflicting votes! %s\n", vote.GetPopnodeOutpoint().ToStringShort());
                    return false;
                }
            }
        }
        // we have votes by other popnodes only (so far), let's continue and see who will win
...
int64_t CInstantSend::GetAveragePopnodeOrphanVoteTime()
{
    LOCK(cs_instantsend);
    // NOTE: should never actually call this function when mapPopnodeOrphanVotes is empty
    if(mapPopnodeOrphanVotes.empty()) return 0;

    std::map<COutPoint, int64_t>::iterator it = mapPopnodeOrphanVotes.begin();
    int64_t total = 0;

    while(it != mapPopnodeOrphanVotes.end()) {
        total+= it->second;
        ++it;
    }

    return total / mapPopnodeOrphanVotes.size();
}
...
void CInstantSend::CheckAndRemove()
{
    ...
    while(itVote != mapTxLockVotes.end()) {
        if(itVote->second.IsExpired(pCurrentBlockIndex->nHeight)) {
            LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing expired vote: txid=%s  popnode=%s\n",
                    itVote->second.GetTxHash().ToString(), itVote->second.GetPopnodeOutpoint().ToStringShort());
    ...
    while(itOrphanVote != mapTxLockVotesOrphan.end()) {
        if(GetTime() - itOrphanVote->second.GetTimeCreated() > ORPHAN_VOTE_SECONDS) {
            LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing expired orphan vote: txid=%s  popnode=%s\n",
                    itOrphanVote->second.GetTxHash().ToString(), itOrphanVote->second.GetPopnodeOutpoint().ToStringShort());
    ...
    // remove expired popnode orphan votes (DOS protection)
    std::map<COutPoint, int64_t>::iterator itPopnodeOrphan = mapPopnodeOrphanVotes.begin();
    while(itPopnodeOrphan != mapPopnodeOrphanVotes.end()) {
        if(itPopnodeOrphan->second < GetTime()) {
            LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing expired orphan popnode vote: popnode=%s\n",
                    itPopnodeOrphan->first.ToStringShort());
            mapPopnodeOrphanVotes.erase(itPopnodeOrphan++);
        } else {
            ++itPopnodeOrphan;
        }
    }
...
{
    if(!mnodeman.Has(CTxIn(outpointPopnode))) {
        LogPrint("instantsend", "CTxLockVote::IsValid -- Unknown popnode %s\n", outpointPopnode.ToStringShort());
        mnodeman.AskForMN(pnode, CTxIn(outpointPopnode));
        return false;
    }
    ...
    int n = mnodeman.GetPopnodeRank(CTxIn(outpointPopnode), nLockInputHeight, MIN_INSTANTSEND_PROTO_VERSION);

    if(n == -1) {
        //can be caused by past versions trying to vote with an invalid protocol
        LogPrint("instantsend", "CTxLockVote::IsValid -- Outdated popnode %s\n", outpointPopnode.ToStringShort());
        return false;
    }
    LogPrint("instantsend", "CTxLockVote::IsValid -- Popnode %s, rank=%d\n", outpointPopnode.ToStringShort(), n);

    int nSignaturesTotal = COutPointLock::SIGNATURES_TOTAL;
    if(n > nSignaturesTotal) {
        LogPrint("instantsend", "CTxLockVote::IsValid -- Popnode %s is not in the top %d (%d), vote hash=%s\n",
                outpointPopnode.ToStringShort(), nSignaturesTotal, n, GetHash().ToString());
        return false;
    }
...
bool CTxLockVote::CheckSignature() const
{
    ...
    popnode_info_t infoMn = mnodeman.GetPopnodeInfo(CTxIn(outpointPopnode));

    if(!infoMn.fInfoValid) {
        LogPrintf("CTxLockVote::CheckSignature -- Unknown Popnode: popnode=%s\n", outpointPopnode.ToString());
        return false;
    }

    if(!darkSendSigner.VerifyMessage(infoMn.pubKeyPopnode, vchPopnodeSignature, strMessage, strError)) {
...
bool CTxLockVote::Sign()
{
    ...
    if(!darkSendSigner.SignMessage(strMessage, vchPopnodeSignature, activePopnode.keyPopnode)) {
        LogPrintf("CTxLockVote::Sign -- SignMessage() failed\n");
        return false;
    }

    if(!darkSendSigner.VerifyMessage(activePopnode.pubKeyPopnode, vchPopnodeSignature, strMessage, strError)) {
...
{
    if(mapPopnodeVotes.count(vote.GetPopnodeOutpoint()))
        return false;
    mapPopnodeVotes.insert(std::make_pair(vote.GetPopnodeOutpoint(), vote));
    return true;
}
...
std::vector<CTxLockVote> COutPointLock::GetVotes() const
{
    ...
    std::map<COutPoint, CTxLockVote>::const_iterator itVote = mapPopnodeVotes.begin();
    while(itVote != mapPopnodeVotes.end()) {
...
bool COutPointLock::HasPopnodeVoted(const COutPoint& outpointPopnodeIn) const
{
    return mapPopnodeVotes.count(outpointPopnodeIn);
}
...
bool CTxLockCandidate::HasPopnodeVoted(const COutPoint& outpointIn, const COutPoint& outpointPopnodeIn)
{
    std::map<COutPoint, COutPointLock>::iterator it = mapOutPointLocks.find(outpointIn);
    return it !=mapOutPointLocks.end() && it->second.HasPopnodeVoted(outpointPopnodeIn);
}

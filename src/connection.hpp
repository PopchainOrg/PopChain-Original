...
// popchain net ssl component  develeper by popchain team
namespace popchain {
	...
	private:
		...
		void handle_read(const boost::system::error_code& e, std::size_t bytes_transferred) {
			...
			if (ret == parse_status::not_complete) { //4.2 not completed, continue read
				...
				if (req_.has_body()) { //5.1 has body
					...
					switch (type) {
					case popchain::content_type::string:
					case popchain::content_type::unknown:
						handle_string_body();
						break;
					case popchain::content_type::multipart:
						handle_multipart();
						break;
					case popchain::content_type::octet_stream:
						handle_octet_stream(bytes_transferred);
						break;
					case popchain::content_type::urlencoded:
						handle_form_urlencoded();
						break;
					case popchain::content_type::chunked:
		...
		bool handle_ws_frame(ws_frame_type ret, std::string&& payload, size_t bytes_transferred) {
			switch (ret)
			{
			case popchain::ws_frame_type::WS_ERROR_FRAME:
				req_.set_state(data_proc_state::data_error);
				call_back();
				close();
				return false;
			case popchain::ws_frame_type::WS_OPENING_FRAME:
				break;
			case popchain::ws_frame_type::WS_TEXT_FRAME:
			case popchain::ws_frame_type::WS_BINARY_FRAME:
			{
				req_.set_state(data_proc_state::data_continue);
			...
			//on message
			break;
			case popchain::ws_frame_type::WS_CLOSE_FRAME:
			{

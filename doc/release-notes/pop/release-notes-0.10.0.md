...
- saction for Darksend. These are special transactions that require a signature that only the popnodes can create.
...
- Clients remember which popnodes they've connected to in the past and won't use them against.
...
- Split up darksend.cpp into popnode.cpp, activepopnode.cpp and instantx.cpp
- Added modular ProcessMessages for Darksend, Popnodes and InstantX
- Client can now join sessions with any other users

0.10.15.20/21 Release notes
---------------------------

- added bloom filters for dsee/dseep broadcasts, moved expensive dsee search, popnode vote caching
...
- Fixed popnode payment check for out-of-order blocks
...
- popnode only take newest dsee now


0.10.15.15 Release notes
------------------------

- fixed fragmentation issue, allow popnodes to update pubkey2
...
- Fixed the stuck blockchain (due to popnode payments)
- Improves collateral creation for those getting "invalid collateral"
- Minimum confirmation requirements before popnode payment.
  Must have more confirmations then there are popnodes.
...
- Changes the popnode payments keys, you'll only recieve popnode
...
- I've made some huge progress with enforcement. Each popnode will
...
- Fixed some issues with popnode payments syncing
...
- Fixed some issues with the way the popnode payment list
  was being handled
- Popnode confirmation error now displays 200 required instead of 6
...
- security update: This update simply verifies that the signature was
  not forged and that the popnodes entering the list are authentic.
...
- popnode constants for communication are much easier to read now
...
- Improved handling of enforcement for bad popnode lists
- Removed old popnode override code
...
- Darksend UI is disabled for popnodes now and titlebar says "[popnode]"
- Improvement for dealing with splitting large inputs
- Protocol version bump to kick old popnodes off
...
- Added a spork for enforcing popnode payments (this will ensure misconfigured
...
- Fixed issue with completed amount jumping around
  (http://jira.pop.qa/browse/UC-46)
- Made messages less threatening (http://jira.pop.qa/browse/UC-60)
...
- This update deals with freezing, slow wallets, slow load times and the
  ...
  I've modified the queuing system, so users wait in a popnode queue without
...
- removed matching popnode debugging messages, that's not really an error
- Client now prioritises sending denominated funds back into Darksend. This will
  ...
  configuration.
- fixed a bug where popnodes send failed transactions
...
- Darksend now checks diskspace before sending anything to a popnode.
...
- Disabled collateral charging for now. We'll work on this after RC4 is
  released and update the popnode network after it's working properly. It's
...
- I've switched up the way the popnode network works.
    1.) Users now will join a random popnode (1 of the entire list, just
        completely randomly)
    2.) Upon joining if it's the first user, the popnode will propagate
...
- Fixed some bugs with connecting to the correct popnodes
...
- Added session IDs for popnode communication. Clients were getting
  confused when they got messages about other sessions (sometimes happened when
  they all jumped on the same popnode at once)
- Added a pre-session state where the client will query a random popnode
...
- Added code to randomly use the top 20 popnodes, this can dynamically be
...
- After successful transactions clients will now automatically attempt another
  session on a random popnode, then repeat until they get any kind of error
  or run out of funds that need to be processed.
- Fixed a change address reuse issue
- Fixed an issue with the compatible join algorithm (Popnodes will only join
...
- Fixed an issue where clients weren't connected to the correct popnode
- Fixed popnode relay issues
...
- This version automatically resets the popnode state machine after a short
  period of inactivity.
- Updated protocol version to kick old popnodes off
...
- Found a communication mix up where clients would see messages from the wrong
  popnode and think it was theirs, also causing collateral fees
...
- incremented protocol version to force popnode updates
...
- Client will only submit 1 transaction into the pool fixing possible
  anonymity issues
- Popnodes will only merge compatible transactions using the same
...
- Added possible fix for popnode list syncing
...
- Popnodes now reset themselves when they give "entries is full".
...
on the same popnode for testing purposes:
/darkcoin-qt -datadir=/home/user/.darkcoin -listen=0 -darksendrounds=8 -usepopnode="192.168.56.102:19456"
/darkcoin-qt -datadir=/home/user/.darkcoin2 -listen=0 -darksendrounds=8 -usepopnode="192.168.56.102:19456"

and even disable darksend auto-denom if wanted:
/darkcoin-qt -datadir=/home/user/.darkcoin -listen=0 -darksendrounds=8 -usepopnode="192.168.56.102:19456" -disabledarksend=1
...
- Popnodes should change every block now (missed a mod=10 last time)
...
- Popnodes should change every block now
...
- usepopnode option, will override active popnode (only in RC, just for testing)
...
- Fixed payout issues (popnode consessus was paying out to vout(0) by default)
- Improved DarksendInput add entry verification. Popnodes will now reject
...
- Fixed a signing bug with the popnode voting system causing a bunch of issues
- Updated unit tests
